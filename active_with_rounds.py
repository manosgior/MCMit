...
def get_error(pd:tuple) -> float:
    active, p, ls_basis, d, idle, rounds = pd
    if active == 0:
        sync = 'passive'
    elif active == 1:
        sync = 'active'
...
if __name__ == '__main__':
    ...
    idles = [500, 1000]
    rounds = range(11)
    pd = list(itertools.product(active, p, ls, ds, idles, rounds))
    p = ['X']
    ls = ['X']
    pd += list(itertools.product(active, p, ls, ds, idles, rounds))
    ...
    import pickle
    with open('active_passive_synchronization_rounds.pkl', 'wb') as file:
