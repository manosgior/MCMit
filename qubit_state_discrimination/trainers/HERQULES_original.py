...
logger = logging.getLogger("herqules")

# Trace length used by all module-level data loading + demodulation. Default
# is 500 (1 µs). Override via HERQULES_TRACE_LENGTH env var so analyze /
# train scripts can target shorter readouts without editing this file:
#   export HERQULES_TRACE_LENGTH=400
TRACE_LENGTH = int(os.environ.get("HERQULES_TRACE_LENGTH", 500))

# --- Subset-stability study controls (read by get_train_val_and_test_set) ---
# When SUBSET_SEED is set, the per-class shot indices are PERMUTED with that
# seed before slicing, so different seeds draw different (fixed-size) subsets
# from the same arrays. SUBSET_NUM_TRAIN_VAL / SUBSET_NUM_TEST pin the split
# sizes (e.g. the paper's 3000 / 7000). All three default to None, which
# reproduces the original deterministic "first-N-per-class" behaviour.
SUBSET_NUM_TRAIN_VAL = None
SUBSET_NUM_TEST      = None
SUBSET_SEED          = None


def get_train_val_and_test_set(trace, y, num_qubits=5, NUM_TRAIN_VAL=None, NUM_TEST=None, NUM_VAL_RATIO = 0.53, trace_length=TRACE_LENGTH):
    ...
    """
    # Subset-study overrides: fall back to module globals when caller passed
    # nothing (the usual case — the internal callers never pass these).
    if NUM_TRAIN_VAL is None:
        NUM_TRAIN_VAL = SUBSET_NUM_TRAIN_VAL
    if NUM_TEST is None:
        NUM_TEST = SUBSET_NUM_TEST

    # 1. Determine exactly how many samples we actually have per class
    class_counts = [np.sum(y == i) for i in range(2**num_qubits)]
    min_available = min(class_counts)

    if NUM_TRAIN_VAL is None or NUM_TEST is None:
        # Default: use all available data per class with the original
        # 30% train+val / 70% test split implied by the old (3000, 7000) defaults.
        NUM_TRAIN_VAL = int(min_available * 0.3)
        NUM_TEST = min_available - NUM_TRAIN_VAL
    elif NUM_TRAIN_VAL + NUM_TEST > min_available:
        scale = min_available / (NUM_TRAIN_VAL + NUM_TEST)
        NUM_TRAIN_VAL = int(NUM_TRAIN_VAL * scale)
        NUM_TEST = min_available - NUM_TRAIN_VAL

    NUM_VAL = int(NUM_TRAIN_VAL * NUM_VAL_RATIO)
    NUM_TRAIN = NUM_TRAIN_VAL - NUM_VAL
    n_classes = 2 ** num_qubits
    if not getattr(get_train_val_and_test_set, "_split_logged", False):
        logger.info(f"Split per class: train={NUM_TRAIN}, val={NUM_VAL}, test={NUM_TEST} "
                    f"(× {n_classes} classes = train {NUM_TRAIN * n_classes}, "
                    f"val {NUM_VAL * n_classes}, test {NUM_TEST * n_classes})")
        get_train_val_and_test_set._split_logged = True
    train_set = []
    val_set = []
    test_set = []
    y_train = []
    y_val = []
    y_test = []
    # Fresh RandomState per call (deterministic given SUBSET_SEED), so every
    # call within one trial produces the SAME shuffle — train/val/test stays
    # consistent across the MF, RMF and Net_rmf stages. Different SUBSET_SEED
    # values draw different fixed-size subsets.
    rng = np.random.RandomState(SUBSET_SEED) if SUBSET_SEED is not None else None
    for i in range(0, 2**num_qubits):
        ...
        y_train.append(np.full(NUM_TRAIN, i))
        y_val.append(np.full(NUM_VAL, i))
        y_test.append(np.full(NUM_TEST, i))
    ...
    return tuple((train_set, val_set, test_set)), tuple((y_train, y_val, y_test))
...
def demodulate_multiplexed_traces(
    ...
    The per-qubit demodulated traces are returned as float32 arrays in a single
    in-memory dict (no HDF5 files are written). Downstream consumers read this
    dict via the module-level ``_DEMOD_CACHE``.
    ...
    # Separate the I and Q components of all traces (float32 keeps the cache small)
    DataI = iq_traces[:, :, 0].astype(np.float32, copy=False)
    DataQ = iq_traces[:, :, 1].astype(np.float32, copy=False)

    if normalize:
        DataI = DataI - np.mean(DataI, axis=1, keepdims=True, dtype=np.float32)
        DataQ = DataQ - np.mean(DataQ, axis=1, keepdims=True, dtype=np.float32)

        corr_factor = (np.std(DataI, axis=1, keepdims=True, dtype=np.float32) /
                       np.std(DataQ, axis=1, keepdims=True, dtype=np.float32))
        DataQ = DataQ * corr_factor

    # Time vector (float32) and Butterworth low-pass design
    vTime = (np.arange(trace_length) * dt).astype(np.float32)
    sos = butter(3, filter_cutoff, btype='low', fs=sampling_rate, output='sos')

    demodulated_qubits = {}
    t_total = time.time()
    logger.info(f"Demodulating {num_traces} traces × {trace_length} samples "
                f"for {len(qubit_frequencies)} qubits...")

    for i, dFreq in enumerate(qubit_frequencies):
        t0 = time.time()
        vCos = np.cos(2 * np.pi * vTime * dFreq).astype(np.float32)
        vSin = np.sin(2 * np.pi * vTime * dFreq).astype(np.float32)

        i_mixed = DataI * vCos + DataQ * vSin
        q_mixed = DataQ * vCos - DataI * vSin

        i_filtered = sosfilt(sos, i_mixed, axis=1).astype(np.float32, copy=False)
        q_filtered = sosfilt(sos, q_mixed, axis=1).astype(np.float32, copy=False)
        del i_mixed, q_mixed

        demodulated_qubits[i] = np.stack((i_filtered, q_filtered), axis=-1)
        del i_filtered, q_filtered
        logger.info(f"  Qubit {i+1}/{len(qubit_frequencies)} demodulated "
                    f"({time.time() - t0:.1f}s, "
                    f"f_IF={dFreq/1e6:+.2f} MHz, "
                    f"shape={demodulated_qubits[i].shape}, "
                    f"{demodulated_qubits[i].nbytes / 1e9:.2f} GB)")

    cache_gb = sum(a.nbytes for a in demodulated_qubits.values()) / 1e9
    logger.info(f"Demodulation complete in {time.time() - t_total:.1f}s, "
                f"in-memory cache size: {cache_gb:.2f} GB")
    return demodulated_qubits

data_path = "/data/five_qubit_data"
# Data file paths. Default to the real chip data; override via env vars to
# point at a synthetic dataset (e.g. generated by runners/generate_synthetic.py)
# without editing the file. Both vars can point at the same .hdf5 — the file
# just needs X_train / y_train / X_test / y_test datasets.
train_file_name = os.environ.get(
    "HERQULES_TRAIN_FILE", "/data/five_qubit_data/DRaw_C_Tr_v0-001"
)
test_file_name = os.environ.get(
    "HERQULES_TEST_FILE", "/data/five_qubit_data/DRaw_C_Te_v0-002"
)
...
def load_hdf5_data(filepath, trace_length, is_test=False, max_samples=None):
    ...
    t0 = time.time()
    logger.info(f"Loading {filepath} (key=X_{key_suffix}, trace_length={trace_length})")
    ...
    logger.info(f"Loaded {X.shape[0]} traces, shape={X.shape}, dtype={X.dtype} "
                f"({X.nbytes / 1e9:.2f} GB) in {time.time() - t0:.1f}s")
    return X, y

X_test_raw, y_test_raw = load_hdf5_data(test_file_name, TRACE_LENGTH, is_test=True, max_samples=MAX_TEST_SAMPLES)
X_train_raw, y_train_raw = load_hdf5_data(train_file_name, TRACE_LENGTH, is_test=False, max_samples=MAX_TRAIN_SAMPLES)

all_data = np.concatenate((X_train_raw, X_test_raw), axis=0)
y_data = np.concatenate((y_train_raw, y_test_raw), axis=0)
logger.info(f"Combined dataset: {all_data.shape[0]} traces, class distribution: "
            f"min={int(np.min(np.bincount(y_data)))}, max={int(np.max(np.bincount(y_data)))}, "
            f"per-class avg={all_data.shape[0] // 32}")
...
_DEMOD_CACHE = demodulate_multiplexed_traces(iq_traces=all_data, qubit_frequencies=freq_readout, sampling_rate=sampling_rate)
...
def get_data(qubit):
    ...
    Reads from the module-level ``_DEMOD_CACHE`` dict populated by
    :func:`demodulate_multiplexed_traces`. The shared global label array
    ``y_data`` is returned unchanged as the label set (all qubits share the
    same experiment shots).
...
class relaxation_mf_classifier(preclassifier):
    ...
    def predict(self, num_qubits=5, data_type=0, trace_length=TRACE_LENGTH):
...
def load_data_all(NUM_TRAIN_VAL=None, NUM_TEST = None, NUM_VAL_RATIO = 0.35):
...
def train_baseline():
    ...
    for epoch in range(0, max_epochs):
        ...
        if acc_val >= best_acc:
            best_acc = acc_val
            T.save(net.state_dict(), os.path.join(path, f'HERQULES_len{TRACE_LENGTH}.pth'))

    print("Finished  training")
    print("Best acc on val dataset: {}".format(best_acc))

    net.load_state_dict(T.load(os.path.join(path, f'HERQULES_len{TRACE_LENGTH}.pth')))
    #net.to(device)
    net.eval()
...
def mf_demux_data_prep(trace_length=TRACE_LENGTH):
...
def train(run_pre_filter=False,
          run_semi_sup=True,
          run_rmf = True,
          train_data_ratio = 1, dur=1000, save=True, standardize=False,
    ...
    logger.info("=== Stage 1: Pre-classifier (geometric trace purification) ===")
    t0 = time.time()
    semi_sup_classifier.fit()
    semi_sup_classifier.save_state('preclassifier_state.pkl')
    logger.info(f"Pre-classifier fit + saved in {time.time() - t0:.1f}s")
    if run_semi_sup:
        filtered_train_data, filtered_labels = semi_sup_classifier.predict(train_semi_sup_data)
        logger.info(f"Semi-supervised filter retained {filtered_train_data.shape[0]} traces")

    fast_readout=20 - int(dur / 50)
    boxcars = [1, 1, 9, 2, 9]
    logger.info(f"Boxcars: {boxcars}")
    trace_classes = None
    if run_rmf:
        logger.info("=== Stage 2: Relaxation matched filter (RMF) ===")
        t0 = time.time()
        trace_classes = semi_sup_classifier.get_traces()
        rmf.fit(trace_classes, boxcars=boxcars)
        rmf.save_state('rmf.pkl')
        logger.info(f"RMF fit + saved in {time.time() - t0:.1f}s")
        t0 = time.time()
        train_rmf_data = rmf.predict(data_type=0)
        val_rmf_data = rmf.predict(data_type=1)
        test_rmf_data = rmf.predict(data_type=2)
        logger.info(f"RMF predictions for train/val/test in {time.time() - t0:.1f}s "
                    f"(train shape={train_rmf_data.shape})")

    best_bc = boxcars
    logger.info("=== Stage 3: Standard matched filter (per-qubit demux split prep) ===")
    t0 = time.time()
    demux_data_train, demux_data_val, demux_data_test = mf_demux_data_prep(TRACE_LENGTH)
    logger.info(f"mf_demux_data_prep done in {time.time() - t0:.1f}s "
                f"(train shape={np.array(demux_data_train).shape})")
    ...
    logger.info(f"MF envelope search complete in {time.time() - t0:.1f}s")

    no_mf = False

    if no_mf and run_rmf:
        train_feat, val_feat, test_feat = train_rmf_data, val_rmf_data, test_rmf_data
    elif run_rmf:
        train_feat = np.concatenate((matched_filter_preprocess_demux(demux_data_train, mf_envelopes, scramble=scramble), train_rmf_data), axis=2)
        val_feat   = np.concatenate((matched_filter_preprocess_demux(demux_data_val,   mf_envelopes, scramble=scramble), val_rmf_data),   axis=2)
        test_feat  = np.concatenate((matched_filter_preprocess_demux(demux_data_test,  mf_envelopes, scramble=scramble), test_rmf_data),  axis=2)
    else:
        train_feat = matched_filter_preprocess_demux(demux_data_train, mf_envelopes, scramble=scramble)
        val_feat   = matched_filter_preprocess_demux(demux_data_val,   mf_envelopes, scramble=scramble)
        test_feat  = matched_filter_preprocess_demux(demux_data_test,  mf_envelopes, scramble=scramble)

    # Optional per-feature standardization (z-score with TRAIN stats), applied
    # to all three splits. Net_rmf has no input normalization and trains at
    # lr=0.01, so large MF magnitudes (e.g. on clean synthetic data, ~1e2)
    # cause gradient explosion -> the model collapses to a constant output
    # (~chance). Standardizing makes training scale-invariant, matching the
    # internal BatchNorm the CNN baseline already has. Default off so real-data
    # behaviour / existing checkpoints are unchanged.
    if standardize:
        flat = train_feat.reshape(-1, train_feat.shape[-1])
        mu = flat.mean(axis=0)
        sd = flat.std(axis=0) + 1e-8
        train_feat = (train_feat - mu) / sd
        val_feat   = (val_feat   - mu) / sd
        test_feat  = (test_feat  - mu) / sd
        logger.info(f"Standardized MF/RMF features (per-feature z-score, train stats)")

    train_ds = MFOutputDataset(train_feat)
    val_ds = MFOutputDataset(val_feat)
    test_ds = MFOutputDataset(test_feat)
    ...
    logger.info(f"=== Stage 4: NN training ({type(net).__name__}, "
                f"max_epochs={max_epochs}, batch={bat_size}, lr={lrn_rate}) ===")
    logger.info(f"Train batches/epoch: {len(train_ldr)}, "
                f"val batches: {len(val_ldr)}, test batches: {len(test_ldr)}")
    t_train = time.time()

    best_acc = -1
    best_state = None
    acc_train_itr, acc_val_itr = [], []
    # Absolute path to the bind-mounted saved_models so the checkpoint persists
    # to the host regardless of cwd. Relative "./saved_models" resolves to
    # /app/Discriminators/saved_models when launched from the Discriminators
    # folder, which is the in-image COPY (not the mount) and is lost on
    # container exit.
    ...
    with T.profiler.profile(
        ...
        for epoch in range(0, max_epochs):
            ...
            if acc_val >= best_acc:
                best_acc = acc_val
                # Keep best state in memory; persist to disk only if save=True.
                # Subset-stability trials pass save=False to avoid clobbering
                # the real checkpoint and to skip the disk round-trip.
                best_state = {k: v.detach().cpu().clone() for k, v in net.state_dict().items()}
                if save:
                    T.save(net.state_dict(), os.path.join(path, f'HERQULES_len{TRACE_LENGTH}.pth'))

    logger.info(f"NN training finished in {time.time() - t_train:.1f}s "
                f"(best val acc: {best_acc:.4f})")

    net.load_state_dict(best_state)
    acc_test, acc_test_per_qubit = accuracy(net, test_ldr)
    logger.info(f"Test acc: {acc_test:.4f} | per qubit: "
                f"{['{:.4f}'.format(a) for a in acc_test_per_qubit]}")
    #print("Acc per qubit: {}".format(acc_test_per_qubit))
    return acc_test_per_qubit
    

if __name__ == "__main__":
    # Only train when this file is run directly. Importing it (e.g. from the
    # subset-stability driver or analyze scripts) must NOT trigger a full
    # training run.
...
def test():
    ...
    """
    model = Net_rmf()
    _ckpt = os.path.join("/app/saved_models", f'HERQULES_len{TRACE_LENGTH}.pth')
    model.load_state_dict(T.load(_ckpt))
    print(_ckpt)
    data_train, data_val, data_test = mf_demux_data_prep(trace_length=TRACE_LENGTH)
    for i in data_test:
        print(i.shape)

    best_bc = [1, 1, 9, 2, 9]
    # NOTE (corrected 2026-06): an earlier comment here claimed "the released
    # code fit MF envelopes on the test set (a data leak)". That is NOT true.
    # Verified against the published notebook (satvikmaurya/HERQULES,
    # HERQULES.ipynb): the original fits envelopes on TRAIN
    # (search_matched_filter_for_all_qubits_demux(demux_data_train)) and reuses
    # the SAME envelopes for val/test -- there is no train/test MF leak upstream.
    # This standalone test() is a porting-only convenience (the published code
    # has no separate test(); it evaluates inside train()). We fit envelopes on
    # data_train here, matching the original, because the MF envelopes are not
    # persisted alongside rmf.pkl. (The real, published defect is the shot-mixing
    # in matched_filter_preprocess_demux, not anything train-vs-test here.)
    mf_envelopes, _ = search_matched_filter_for_all_qubits_demux(data_train, best_bc=best_bc)
    ...
    test_rmf_data = rmf.predict(data_type=2, trace_length=TRACE_LENGTH)
