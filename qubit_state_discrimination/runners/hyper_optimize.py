...
NUM_QUBITS = 5
# Only hyper-optimize the final (longest) trace length. The shorter lengths are
# already done; this run targets just the 500-sample point.
TRACE_LENGTHS = [500]
# Search space shortened: fewer trials, since the per-objective ranges below are
# now tightened around where the best trials historically land (see CSV reports).
N_OPTUNA_TRIALS = 15
EPOCHS_PER_TRIAL = 30
...
def objective_cnn(trial, trace_length):
    ...
    model = CNN(in_channels=10, m_param=16, num_qubits=NUM_QUBITS).to(DEVICE)
...
def objective_klinq_teacher(trial, X_raw, y_raw, trace_length, target_qubit):
    ...
    """
    # Shortened search space: best teacher trials historically pick lr ~1e-4..7e-4
    # (median 1.8e-4) and batch >=128, so the upper lr and the bs=64 region are dropped.
...
def objective_klinq_student(trial, X_raw, y_raw, trace_length, target_qubit, teacher_model):
    ...
    """
    # Shortened search space: best student trials cluster at lr ~2.5e-3 (median),
    # batch <=256, mid temperature, and low/mid alpha. Tightening the categorical
    # grid from 4x5x6=120 to 3x3x3=27 lets the few trials actually cover it.
    lr = trial.suggest_float("lr", 3e-4, 5e-3, log=True)
    batch_size = trial.suggest_categorical("batch_size", [64, 128, 256])
    temperature = trial.suggest_categorical("temperature", [2, 3, 4])
    alpha = trial.suggest_categorical("alpha", [0.2, 0.5, 0.8])

    target_length = 5  # averaging target bins per channel
    y_q = extract_qubit_labels(y_raw, target_qubit)

    # --- Build student feature vector ---
    # Paper-faithful KLiNQ student: input is [binned avg | MF scalar] only.
    # The flat trace is only used internally to compute the MF envelope and
    # scalar — it is NOT fed to the student (that would give the student the
    # same information as the teacher and balloon fc1 to ~16k weights).
    X_flat = X_raw.reshape(X_raw.shape[0], -1).copy()

    # 1) Time-averaged trace (downsample each IQ channel to target_length bins)
    ...
    # 2) Matched-filter scalar for this qubit
    ...
    X_student = np.concatenate([_znorm(X_avg), _znorm(mf_scalar)], axis=1)
    ...
    for length in TRACE_LENGTHS:
        ...
        if "KLiNQ_teacher" in models_to_run or "KLiNQ_student" in models_to_run:
            ...
            for qubit in range(NUM_QUBITS):
                ...
                # Paper-faithful: drop the flat trace; student sees only avg + MF.
                X_combined_test = np.concatenate(
                    [_znorm(X_avg_test), _znorm(mf_scalar_test)], axis=1
                )
...
if __name__ == "__main__":
    optimize_models(models_to_run=["KLiNQ_student"])
