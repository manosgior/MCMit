...
class circuit(circuit_parser, gate_lib, stat_counter, stim_integ):
    def __init__(self, distance:int=3, disable_noise:bool=False, seed:int=0,
        ...
        self.xobs = []
        if sync == 'active':
            self.add_distributed_idle_periods(0, sync_idle, round=sync_round)
        elif sync == 'passive':
            self.add_pre_decode_barrier(0, sync_idle, sync_round)
        ...
        for interval in intervals:
            ...
            if self.disable_noise == False:
                ...
                new_ckt = np.insert(new_ckt, 
                                    ...
                                    post_noise)
                idx_offset += 1
                if self.distributed_idle and round_counter == self.sync_round - 1 and layers > 0:
                    # if round_counter == self.rounds_per_op and layers == 1:
                    #     self.distributed_idle = False
                    layers -= 1
                    new_ckt = np.insert(new_ckt,
                                        interval[1] + idx_offset,
...
if __name__ == "__main__":
    ...
    sim = circuit(distance=d, num_patches_y=20, num_patches_x=20, spacing=1, disable_noise=False, fixed_t1=250, fixed_t2=150, fixed_cnot_latency=200, fixed_measure_latency=500, fixed_cnot_noise=err, fixed_measure_noise=err, rounds_per_op=d+1, idle_multiplier=3, basis=basis, ls_basis=ls_basis, merge=True, sync='passive', sync_idle=slack, sync_round=d+1).from_string('qreg q[2];')
