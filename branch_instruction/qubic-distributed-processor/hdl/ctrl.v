...
*       FPROC type (alu_fproc, jump_fproc, jump_reduce_fproc):
...
module ctrl#(
    ...
    //in general: byte 1 of opcode is opcode below, byte 0 ALU opcode
    
    //5-bit opcode: 4-bit operation followed by LSB select for ALU_IN1 (0 for cmd, 1 for reg)
    parameter REG_ALU = 4'b0001; //|opcode[16]|cmd_value[32]|reg1_addr[4]|reg_write_addr[4]
    parameter JUMP_I = 4'b0010; //|opcode[16]|cmd_value[32]|reg_addr[4]
    ...
    assign alu_opcode = opcode[7:0];
    assign alu_in0_sel = opcode[8];
    ...
    always @(*) begin
        if(state == MEM_WAIT_STATE) begin
            ...
            case(opcode[12:9])
                ...
                ALU_FPROC, JUMP_FPROC, JUMP_REDUCE_FPROC : begin
            ...
            case(opcode[12:9])
                ...
                JUMP_COND, JUMP_FPROC, JUMP_REDUCE_FPROC : begin
                    mem_wait_rst = 1; 
