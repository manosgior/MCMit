...
@cocotb.test()
async def multiple_meas_test(dut):
    cocotb.start_soon(generate_clock(dut))
    dut.reset.value = 1
    await RisingEdge(dut.clk)
    dut.reset.value = 0
    dut.fproc_enable.value = 0
    dut.meas.value = 0b11010
    dut.meas_valid.value = 0b11111
    await RisingEdge(dut.clk)
    dut.fproc_enable.value = 1
    # An ID >= N_MEAS requests all measurements at once
    dut.fproc_id[0].value = 7
    await RisingEdge(dut.clk)
    await RisingEdge(dut.clk)
    await RisingEdge(dut.clk)
    assert dut.fproc_ready.value == 1
    assert dut.fproc_data[0].value == 0b11010
