...
async def cmd_mem_out_test(dut):
    ...
    for i in range(n_cmd):
        cmd_list.append(random.randint(0,2**120-1) + (1<<129))
...
async def pulse_freq_trig_test(dut):
    ...
    for i in range(n_cmd):
        ...
        cmd_list.append((pulse_i_opcode << 125) + ((freq_word + 2**10) << 60) + (pulse_time_list[i] << (5)))
...
async def regwrite_i_test(dut):
    ...
    cmd = (0b00010000 << 125) + (reg_val << 88) + (reg_addr << 80)
...
@cocotb.test()
async def reg_logic_i_test(dut):
    """
    Write a value to a random register. Then, perform a logic operation
    on that register and an intermediate value, and store in another register.
    Try this 100 times w/ random values and ops.
    """
    for i in range(100):
        ...
        reg_addr0 = random.randint(0,15)
        reg_addr1 = random.randint(0,15)
        reg_val = random.randint(0, 2**31-1)
        ival = random.randint(0, 2**31-1)
        op = random.choice(['and', 'or', 'xor', 'maj'])
        ...
        dut._log.debug('i val in: {}'.format(ival))
        dut._log.debug('op: {}'.format(op))
        dut._log.debug('val out: {}'.format(reg_read_val.integer))
        dut._log.debug('correct val out: {}'.format(correct_val))
...
@cocotb.test()
async def jump_reduce_fproc_i_test(dut):
    cmd_list = []
    fproc_max_t = 20
    jump_addr = random.randint(0, 2**8-1)
    ...
    #cmd_list.append(cg.alu_cmd('reg_alu', 'i', cmp_ival, 'id0', 0, regwrite_addr))
    op = random.choice(['le', 'ge', 'eq'])
    cmd_list.append(cg.alu_cmd('jump_reduce_fproc', 'i', cmp_ival, op, jump_cmd_ptr=jump_addr))
...
def evaluate_alu_exp(in0, op, in1):
    if op == 'add':
        ...
        return in1
    # For the logic operations in0 is the mask, in1 the measured bits
    elif op == 'and':
        return (in0 & in1) == in0
    elif op == 'or':
        return (in0 & in1) != 0
    elif op == 'xor':
        # True if uneven number of 1s
        bits = in0 & in1
        num_1s = 0
        for _ in range(32):
            if bits & 1:
        ...
        print(f"data: {num_1s_data}, mask: {num_1s_mask}")

        return num_1s_data >= ((num_1s_mask // 2) + 1)
