...
def cmdparse(cmdbuf):
    ...
    parsed_cmds=[]
    for i in range(len(cmd)//5):
        cmd160=0xffffffffffffffffffffffffffffffffffffffff
        for j in [4,3,2,1,0]:
            cmd160=((cmd160<<32)&0xffffffffffffffffffffffffffffffffffffffff)+cmd[i*5+j]
        opcode=(cmd160>>128)&0x1f
        cmdtime=(cmd160>>pulse_field_pos['cmd_time'])&0xffffffff
        cfg=(cmd160>>pulse_field_pos['cfg'])&0xf
        amp=(cmd160>>pulse_field_pos['amp'])&0xffff
        freq=(cmd160>>pulse_field_pos['freq'])&0x1ff
        phase=(cmd160>>pulse_field_pos['phase'])&0x1ffff
        env_word=(cmd160>>pulse_field_pos['env_word'])&0xffffff
