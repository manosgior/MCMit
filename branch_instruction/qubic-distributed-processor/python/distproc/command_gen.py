...
opcodes = {'pulse_write' : 0b10000,
           ...
           'jump_fproc' : 0b01011,
           'jump_reduce_fproc_i' : 0b11010,
           'jump_reduce_fproc' : 0b11011,
           'inc_qclk_i' : 0b01100,
           'inc_qclk' : 0b01101,
           'sync' : 0b01110,
           'done' : 0b10100,
...
def reg_alu_i(value, alu_op, reg_addr, reg_write_addr):
    """
    Returns 160-bit command corresponding to:
    ...
    -------
        cmd : int
            160 bit command
    """
    opcode = (opcodes['reg_alu_i'] << 8) + alu_opcodes[alu_op]
...
def jump_cond_i(value, alu_op, reg_addr, instr_ptr_addr):
    ...
    opcode = (opcodes['jump_cond_i'] << 8) + alu_opcodes[alu_op]
...
def jump_fproc(func_id, alu_reg_addr, alu_op, instr_ptr_addr):
    opcode = (opcodes['jump_fproc'] << 8) + alu_opcodes[alu_op]
    return (opcode << 120) + (alu_reg_addr << 116) + (instr_ptr_addr << 76) + (func_id << 52)

def jump_fproc_i(func_id, value, alu_op, instr_ptr_addr):
    opcode = (opcodes['jump_fproc_i'] << 8) + alu_opcodes[alu_op]
    return (opcode << 120) + (value << 88) + (instr_ptr_addr << 76) + (func_id << 52)

def jump_reduce_fproc(func_id, alu_reg_addr, alu_op, instr_ptr_addr):
    opcode = (opcodes['jump_reduce_fproc'] << 8) + alu_opcodes[alu_op]
    return (opcode << 120) + (alu_reg_addr << 116) + (instr_ptr_addr << 76) + (func_id << 52)

def jump_reduce_fproc_i(func_id, value, alu_op, instr_ptr_addr):
    opcode = (opcodes['jump_reduce_fproc_i'] << 8) + alu_opcodes[alu_op]
    return (opcode << 120) + (value << 88) + (instr_ptr_addr << 76) + (func_id << 52)
...
def alu_cmd(optype, im_or_reg, alu_in0, alu_op=None, alu_in1=0, write_reg_addr=None, jump_cmd_ptr=None, func_id=None):
    ...
    This is a general function for generating the following types of instructions:
        reg_(i)alu, jump_cond(i), alu_fproc(i), jump_fproc(i), jump_reduce_fproc(i), inc_qclk(i)

    Parameters
    ----------
        optype : str
            one of: reg_alu, jump_cond, alu_fproc, jump_fproc, jump_reduce_fproc
        im_or_reg : str
    ...
    if optype in ['reg_alu', 'jump_cond']: #these have alu_in1 from reg
        cmd += alu_in1 << 84
    if optype in ['alu_fproc', 'jump_fproc', 'jump_reduce_fproc']:
        if func_id is not None:
            cmd += func_id << 52
    if optype in ['jump_cond', 'jump_fproc', 'jump_reduce_fproc']:
        cmd += jump_cmd_ptr << 68
    ...
    opcode = (opcodes[opkey] << 8) + alu_opcodes[alu_op]
    cmd += opcode << 120
...
def idle(cmd_time):
    ...
    cmd += opcodes['idle'] << 128
    return cmd

def done_cmd():
    return opcodes['done'] << 128

def pulse_reset():
    return opcodes['pulse_reset'] << 128

def sync(barrier_id):
    return (opcodes['sync'] << 128) + (barrier_id << 112)

def get_phase_word(phase: float) -> int:
    """
    ...
    int:
        17-bit phase word
    """
    phase = int(((phase % (2 * np.pi)) / (2 * np.pi)) * 2 ** pulse_field_widths['phase'])
    return phase % 2 ** pulse_field_widths['phase']

def get_amp_word(amplitude: float) -> int:
    """
    ...
    """
    return int(twos_complement(amplitude * (2 ** (pulse_field_widths['amp'] - 1) - 1), nbits=pulse_field_widths['amp']))
