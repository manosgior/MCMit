...
class FlattenProgram(Pass):
    ...
    def _flatten_control_flow(self, program, label_prefix=''):
        ...
        for i, statement in enumerate(program):
            ...
            if statement.name in ['branch_fproc', 'branch_reduce_fproc', 'branch_var']:
                ...
                elif statement.name == 'branch_reduce_fproc':
                    jump_statement = iri.JumpReduceFproc(alu_cond=statement.alu_cond, cond_lhs=statement.cond_lhs,
                                                   func_id=statement.func_id, scope=statement.scope, jump_label=None)
                else:
...
class MakeBasicBlocks(Pass):
    ...
    def run_pass(self, ir_prog: IRProgram):
        ...
        block_terminators = ['jump_fproc', 'jump_reduce_fproc', 'jump_cond', 'jump_label', 'block_start', 'block_end']
        ...
        for i, statement in enumerate(full_program):
            if statement.name in block_terminators:
                ...
                if statement.name in ['jump_fproc', 'jump_reduce_fproc', 'jump_cond']:
            ...
            elif statement.name in ['branch_fproc', 'branch_reduce_fproc', 'branch_var', 'loop', 'block']:
    ...
    *experimental*, use at your own risk. When in doubt just scope manually.
    ...
    def scope_body(self, instructions: List) -> set:
        ...
        for instr in instructions:
            if instr.name in ['branch_fproc', 'branch_reduce_fproc', 'branch_var', 'loop']:
                if instr.name == 'loop':
...
class GenerateCFG(Pass):
    ...
    def run_pass(self, ir_prog: IRProgram):
        ...
        for blockname in ir_prog.blocknames_by_ind:
            ...
            if block['instructions'][-1].name in ['jump_fproc', 'jump_reduce_fproc', 'jump_cond']:
        ...
        for nodename in nx.topological_sort(ir_prog.control_flow_graph):
            ...
            while i < len(instructions):
                ...
                if isinstance(instr, iri.ReadFproc) or isinstance(instr, iri.JumpFproc) \
                        or isinstance(instr, iri.JumpReduceFproc) or isinstance(instr, iri.AluFproc):
    ...
    def run_pass(self, ir_prog: IRProgram):
        for nodename in nx.topological_sort(ir_prog.control_flow_graph):
            ...
            for instr in instructions:
                ...
                elif instr.name in ['jump_cond', 'jump_fproc', 'jump_reduce_fproc']:
...
class OptimizeALU(Pass):
    ...
    def __init__(self):
        ...
        self._usage_instrs = ['pulse', 'loop', 'jump_fproc', 'jump_reduce_fproc', 'jump_cond'] # instructions that use vars
    ...
    def run_pass(self, ir_prog: IRProgram):
        for nodename in nx.topological_sort(ir_prog.control_flow_graph):
            ...
            while i < len(instructions):
                ...
                elif instr.name == 'jump_fproc' or instr.name == 'jump_reduce_fproc' or instr.name == 'jump_cond':
...
class Schedule(Pass):
    ...
    def _resolve_hold_and_increment(self, instructions: list, scope: set, start_time: int, dest_last_t: dict):
        ...
        while i < len(instructions):
            ...
            elif instr.name in ['jump_fproc', 'jump_reduce_fproc', 'read_fproc', 'alu_fproc']:
                for grp in self._core_scoper.get_groups_bydest(instr.scope):
