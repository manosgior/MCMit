"""Distributed processor assembly language definition:
    ...
    register-based instructions:
        ...
        jump_reduce_fproc:
            {'op': 'jump_reduce_fproc', 'in0': <int, regname>, 'alu_op': alu_op, 'jump_label': jump_label, 'func_id': func_id}
        alu_fproc:
...
class SingleCoreAssembler:
    ...
    def from_list(self, cmd_list):
        for i, cmd in enumerate(cmd_list):
            ...
            elif cmd['op'] in ['reg_alu', 'jump_cond', 'alu_fproc', 'jump_fproc', 'jump_reduce_fproc']:
    ...
    def add_alu_cmd(self, op: str, in0: int | str, alu_op: str, in1_reg: str = None,
        ...
        assert op in ['reg_alu', 'jump_cond', 'alu_fproc', 'jump_fproc', 'jump_reduce_fproc', 'inc_qclk']
        ...
        if op in ['jump_cond', 'jump_fproc', 'jump_reduce_fproc']:
            assert jump_label is not None
            cmd['jump_label'] = jump_label

        if op in ['alu_fproc', 'jump_fproc', 'jump_reduce_fproc']:  # None defaults to 0, implies func_id not used
            cmd['func_id'] = func_id
        else:
            assert func_id is None
    ...
    def add_jump_reduce_fproc(self, in0, alu_op, jump_label, func_id=None, label=None):
        self.add_alu_cmd('jump_reduce_fproc', in0, alu_op, jump_label=jump_label, func_id=func_id, label=label)
    ...
    def get_program_binaries(self):
        ...
        for cmd in self._program:
            ...
            if cmd['op'] == 'pulse':
                ...
                cmd_buf += cg.pulse_cmd(**pulseargs).to_bytes(20, 'little')

            elif cmd['op'] in ['reg_alu', 'jump_cond', 'alu_fproc', 'jump_fproc', 'jump_reduce_fproc', 'inc_qclk']:
                ...
                cmd_buf += cmd_raw.to_bytes(20, 'little')

            elif cmd['op'] == 'jump_i':
                cmd['jump_addr'] = cmd_label_addrmap[cmd['jump_label']]
                cmd_buf += cg.jump_i(cmd['jump_addr']).to_bytes(20, 'little')

            elif cmd['op'] == 'pulse_reset':
                cmd_buf += cg.pulse_reset().to_bytes(20, 'little')

            elif cmd['op'] == 'idle':
                cmd_buf += cg.idle(cmd['end_time']).to_bytes(20, 'little')

            elif cmd['op'] == 'done_stb':
                cmd_buf += cg.done_cmd().to_bytes(20, 'little')
    ...
    def _resolve_fproc_chans(self, single_core_program):
        ...
        for statement in single_core_program:
            if statement['op'] == 'alu_fproc' or statement['op'] == 'jump_fproc' \
                    or statement['op'] == 'jump_reduce_fproc':
    ...
    def get_assembled_program(self):
        ...
        -------
            assembled_prog : dict
                ...
                values : dict
                    'cmd_list' : list of proc commands (160-bit wide)
