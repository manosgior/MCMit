...
def test_branch_fproc():
    qchip = qc.QChip('qubitcfg.json')
    fpga_config = hw.FPGAConfig

    program = [{'name': 'declare', 'var': 'x', 'scope': ['Q0'], 'dtype': 'int'},
               {'name': 'set_var', 'var': 'x', 'value': 5},
               {'name': 'branch_fproc', 'alu_cond': 'eq', 'cond_lhs': 5, 'func_id': 'x',
    ...
    with open('test_outputs/test_opt_branch_fproc.txt', 'r') as f:
        filein = f.read().rstrip('\n')
    ...
    try:
        ...
        with open('test_outputs/test_opt_branch_fproc_err.txt', 'w') as ferr:
            ferr.write(compiler.ir_prog.serialize())

        raise err

# Currently the same behavior as branch_fproc
def test_branch_reduce_fproc():
    qchip = qc.QChip('qubitcfg.json')
    fpga_config = hw.FPGAConfig

    program = [{'name': 'declare', 'var': 'x', 'scope': ['Q0'], 'dtype': 'int'},
               {'name': 'set_var', 'var': 'x', 'value': 5},
               {'name': 'branch_reduce_fproc', 'alu_cond': 'eq', 'cond_lhs': 5, 'func_id': 'x',
                'true': [{'name': 'X90', 'qubit': ['Q0']}],
                'false': [{'name': 'X90', 'qubit': ['Q1']}]}]
    compiler = cm.Compiler(program)
    ...
    with open('test_outputs/test_opt_branch_reduce_fproc.txt', 'r') as f:
        filein = f.read().rstrip('\n')
    ...
    try:
        ...
        with open('test_outputs/test_opt_branch_reduce_fproc_err.txt', 'w') as ferr:
            ferr.write(compiler.ir_prog.serialize())
