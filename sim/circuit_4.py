...
# Will perform lattice surgery and perform a logical measurement of the merged patch
class circuit(circuit_parser, gate_lib, stat_counter, stim_integ):
    def __init__(self, distance:int=3, disable_noise:bool=False, seed:int=0,
        ...
        self.basis = 'Z'
        if self.ls_basis == 'X': # just for logical measurement
            self.basis = 'Z'
        else:
            self.basis = 'X'
        self.exp_type = basis
        ...
        if pre_ls_rounds == 0:
            self.pre_ls_rounds = rounds_per_op
        else:
            self.pre_ls_rounds = pre_ls_rounds
        if post_ls_rounds == 0:
            self.post_ls_rounds = rounds_per_op
        else:
            self.post_ls_rounds = post_ls_rounds
        self.rounds_per_op = rounds_per_op
        if self.pre_ls_rounds > sync_round:
            self.sync_round = self.pre_ls_rounds
        else:
            self.sync_round = sync_round
        if sync == 'active':
            self.add_distributed_idle_periods(0, sync_idle, round=self.sync_round)
        elif sync == 'passive':
            self.add_pre_decode_barrier(0, sync_idle, self.sync_round)
        ...
        gate_lib.__init__(self, distance, num_patches_x, num_patches_y,
                          ...
                          idle_multiplier=idle_multiplier, basis=self.basis,
    ...
    def add_gate(self, gate:str, phys_qubit:int, target_qubit:int=None) -> str:
        ...
        if target_qubit != None:
            ...
            if self.extra_errors and not self.disable_noise:
                noise = 'PAULI_CHANNEL_1(%f, %f, %f) %i %i\n'%(0.0001, 0.0001, 0.0001, target_qubit, phys_qubit)
                str += noise + '\n'
                pass
    ...
    def add_observables(self, mappings:dict, qubit:int) -> str:
        ...
        z_measures = mappings['z_measure_coords']
        x_measures = mappings['x_measure_coords']
        ckt_str = ''
        if self.exp_type != self.ls_basis:
            if self.basis == 'Z':
                indices = {i:self.measurement_tracker.index(i) - len(self.measurement_tracker) 
            ...
            coords = []
            if self.basis == 'Z':
                # Z in horizontal direction, X in vertical
                lim0 = self.d
                lim1 = self.d * 4 + 1 - int(self.d/2) * 2
                for i in z_measures:
                    if i[1] > lim0 and i[1] < lim1:
                        coords.append(i)
                pass
            else:
                lim0 = self.d
    ...
    def setup_patches(self, coords:dict) -> str:
        ...
        for mappings in coords.values():
            ...
            pass
        for _ in range(self.pre_ls_rounds - 1): # Unroll the loop for all rounds
            ckt_str += rep_ckt
        # ckt_str += '}\n'
        ...
        if merge:
            ...
            for _ in range(self.post_ls_rounds - 1):
                ckt_str += rep_ckt
            coords = new_coords
        elif self.repeat == True:
            for _ in range(self.pre_ls_rounds): # Unroll the loop for all rounds
                ckt_str += rep_ckt
        ...
        ckt_str = self.fenced_sync(ckt_str)
        return ckt_str
    ...
    def add_distributed_idle_periods(self, patch:int, duration:float, k:int=1, round:int=None) -> None:
    ...
    def __get_distributed_idle(self, layers:int) -> None:
        ...
        measures = [i for i in self.data_idxs if i in self.patch_qubit_mapper[self.distributed_idle_patch]]
        self.distributed_idle_periods = '\n'.join(self.idling_model(self.distributed_idle_dur / (layers),
                                                                     i_c(i), 
                                                                     i) 
                                                  for i in measures)
        # self.distributed_idle_periods += '\n' + 'R ' + ' '.join(str(i) for i in self.measure_idxs)
        return
    
    def add_pre_decode_barrier(self, patch:int, duration:float, round:int=None) -> None:
        self.passive_sync = True
        self.passive_idle_patch = patch
        self.passive_idle_dur = duration
        if round != None:
            self.sync_round = round
        return
    
    def __get_passive_idle(self) -> None:
        # Emulates the case where patches wait for all other patches to start the decode stage
        self.pre_decode = True
        i_c = lambda i: self.index_coords_mapper[i]
        qubits = [i for i in self.data_idxs if i in self.patch_qubit_mapper[self.passive_idle_patch]]
        self.pre_decode_idle = '\n'.join(self.idling_model(self.passive_idle_dur, i_c(i), i) 
                                         for i in qubits) #self.patch_qubit_mapper[patch])
        return
    ...
    def runahead_sync(self, ckt:str) -> str:
        ...
        def get_idle_periods(sorted_gates:list):
            ...
            for gate, time in sorted_gates:
                ...
                for qubit in list(qubits):
                    ...
                    t = self.time + time - duration
                    timesheet[qubit] = [tuple((t, self.time + time + skew))] \
                        if qubit not in timesheet.keys() \
                        else timesheet[qubit] + [tuple((t, self.time + time + skew))]
                    insts_debug[t] = [gate] \
                        if t not in insts_debug.keys() \
                            else insts_debug[t] + [gate]
                    pass
                pass
            ...
            for q in timesheet.keys():
                ...
                idle_times[q] = np.sum([i[0][1] - i[1][0] + self.decode_latency
                                        for i in exec_intervals]) + self.skew_map[int(q)]
                pass
            ...
            round_counter += 1
            addl_latency = self.decode_latency if round_counter % self.sync_round == 0 else 0
        ...
        for interval_idx, interval in enumerate(intervals):
            ...
            for snip_idx, line in enumerate(snip):
                ...
                if gate in self.measures and final_stage == False:
                    ...
                    final_stage = True
                    pass
                elif final_stage and (gate == 'H' or gate in self.measures_logical):
                    # Measurements complete, new cycle has started
                    # Start a new DAG, complete the last one and update timers
                    final_stage = True if gate in self.measures_logical else False
                    if last_annotation == False:
                        if round_counter >= self.sync_round - self.distributed_rounds and self.distributed_idle:
                        ...
                        if self.disable_noise == False:
                            if self.distributed_idle and round_counter == int((self.sync_round)-1):
                                new_ckt = np.insert(new_ckt,
                                                    intervals[frame_start][0] + idx_offset + 1,
                                                    self.distributed_idle_periods)
                                idx_offset += 1
                                new_ckt = np.insert(new_ckt,
                                                    intervals[frame_end][1] + idx_offset,
                                                    self.distributed_idle_periods)
                                idx_offset += 1
                            new_ckt = np.insert(new_ckt, 
                        ...
                        last_annotation = True if gate in self.measures_logical else last_annotation
                    pass
        ...
        if self.pre_decode and self.disable_noise == False:
            ...
            idx_offset += 1
            pass
        # if self.distributed_idle and self.disable_noise == False:
        #     self.add_pre_decode_barrier(0, self.decode_latency)
        #     new_ckt = np.insert(new_ckt,
        #                         intervals[-1][0] + idx_offset + 1,
        #                         self.pre_decode_idle)
        #     idx_offset += 1
    ...
    def fenced_sync(self, ckt:str) -> str:
        ...
        for interval in intervals:
            ...
            layers = self.pre_ls_rounds * 2
            if self.distributed_idle:
                self.__get_distributed_idle(layers)
            if self.passive_sync and self.pre_decode == False:
                self.__get_passive_idle()
            ...
            if self.disable_noise == False:
                ...
                new_ckt = np.insert(new_ckt, 
                                    ...
                                    post_noise)
                idx_offset += 1
                if last_gate in ['MR'] and self.distributed_idle and round_counter <= self.sync_round - self.distributed_rounds + 1:
                    # sync_counter += 1
                    # if sync_counter == layers:
                    #     self.distributed_idle = False
                    idle = None
                    i_c = lambda i: self.index_coords_mapper[i]
                    idle =  '\n'.join(self.idling_model(self.distributed_idle_dur / layers,
                                                        i_c(i),
                                                        i) for i in self.data_idxs)
                    idle = self.distributed_idle_periods
                    new_ckt = np.insert(new_ckt,
                                        frame_start + idx_offset + 1,
                                        idle)
                    idx_offset += 1
                    new_ckt = np.insert(new_ckt,
                                        interval[1] + idx_offset,
                                        idle)
                    idx_offset += 1
                if last_gate in ['MR'] and round_counter == self.sync_round and self.pre_decode:
                    new_ckt = np.insert(new_ckt,
                                        interval[1] + idx_offset,
                                        self.pre_decode_idle)
                    idx_offset += 1
...
if __name__ == "__main__":
    ...
    basis = 'X'
    slack = 0.5 * 10 ** 6
    sim = circuit(distance=d, num_patches_y=20, num_patches_x=20, spacing=1, disable_noise=False, fixed_t1=4 * 10**6, fixed_t2=1.5*10**6, fixed_cnot_latency=200*10**3, fixed_measure_latency=1 * 10**6, fixed_cnot_noise=err, fixed_measure_noise=0.002, rounds_per_op=d+1, idle_multiplier=3, basis=basis, ls_basis=ls_basis, merge=True, sync='active', sync_idle=slack, sync_round=d+1).from_string('qreg q[2];')
    
    e, _ = sim.get_error_rate(ckt=sim.ckt, num_shots=1_000_000)
    # print(new_ckt)
    # print(sim.skew_map)
    print(sim.time, sim.total_idle, e)
    
    sim = circuit(distance=d, num_patches_y=20, num_patches_x=20, spacing=1, disable_noise=False, fixed_t1=4 * 10**6, fixed_t2=1.5*10**6, fixed_cnot_latency=200*10**3, fixed_measure_latency=1 * 10**6, fixed_cnot_noise=err, fixed_measure_noise=0.002, rounds_per_op=d+1, idle_multiplier=3, basis=basis, ls_basis=ls_basis, merge=True, sync='passive', sync_idle=slack, sync_round=d+1).from_string('qreg q[2];')
